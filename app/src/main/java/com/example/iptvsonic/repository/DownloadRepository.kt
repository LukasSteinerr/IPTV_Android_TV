...
import com.example.iptvsonic.service.DownloadService
...
class DownloadRepository(private val context: Context) {
    ...
    private fun restoreActiveDownloads() {
        scope.launch {
            ...
            val activeDownloads = downloadBox.query()
                .equal(com.example.iptvsonic.model.DownloadedMovie_.status, DownloadedMovie.STATUS_DOWNLOADING.toLong())
                .build()
                .find()
    ...
    fun downloadMovie(movie: Movie, permissionCallback: NotificationPermissionCallback? = null) {
        ...
        val existing = downloadBox.query()
            .equal(com.example.iptvsonic.model.DownloadedMovie_.movieId, uniqueId, QueryBuilder.StringOrder.CASE_SENSITIVE)
            .build()
            .findFirst()
        if (existing != null) {
